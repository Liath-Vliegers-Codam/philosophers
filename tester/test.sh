...
eat_test () {
	...
	do
		...
		if [[ $key == "" ]] ; then
			printf "\nRunning test...\n"
			$1 $input > output.txt 2>&1  # Run philo and redirect output to file
			eat_count=$(grep "is eating" output.txt | wc -l)  # Count occurrences of "is eating"
			printf "\n${GREEN}Eating occurrences:${RESET} $eat_count\n"
		else
			exit 0
		fi
	done 3< ./eat.txt   # open file is assigned fd 3
	exec 3<&-	# close fd 3
}
...
choose_test() {
    read -rn1 -p $'\nChoose test to run:\n\t[0] all tests\n\t[1] eat test\n\t[2] die tests\n\t[3] no-die tests (can take a while)\n\t[ESC] exit tester\n\n' choice
    printf "\n"
    case $choice in
        0) eat_test "$1" && die_test "$1" && no_die_test "$1" ;;
        1) eat_test "$1" ;;
		2) die_test "$1" ;;
        3) no_die_test "$1" ;;
        $'\e') exit 0 ;;
