...
[x] destroy fork and phil mutextes is one too many
[ ] No two different philo routines!
[x] death check checkt of hij dood gaat, niet of hij dood is...
[ ] times to eat can be 0
[x] makefile aanpassen
[ ] print amount of times eaten when done with eating (only for debugging maybe);
[ ] 100 210 100 100 moet kunnen zonder iemand dood te gaan
[ ] - niet zo vaak checken of ze dood gaan
[x] - usleep (1000) in dinner party
[ ] check data races met f-sanitize
...
memset
	...
	Parameters:
		...
		c: Value to fill (converted to an unsigned char).
		n: Number of bytes to fill.
	Usage: 		Initialize memory to a specific value (e.g., 0 for zeroing arrays).	
	Example:	char buffer[10];
				memset(buffer, 0, sizeof(buffer)); // Fill buffer with 0s
	
printf
	Prototype: 	int printf(const char *format, ...);
	Purpose: 	Prints formatted output to the standard output (usually the terminal).
	Parameters:
		format: A format string with placeholders (e.g., %d, %s) for additional arguments.
		Additional arguments correspond to the placeholders.
	Example:	printf("Hello, %s! You have %d messages.\n", "User", 5);
	
malloc
	Prototype:	void *malloc(size_t size);
	Purpose:	Allocates a block of memory dynamically on the heap.
	Parameters:
		size: Number of bytes to allocate.
	Returns:	A pointer to the allocated memory, or NULL on failure.
	Usage:		Used to create dynamic arrays or structures.
	Example:	int *array = (int *)malloc(10 * sizeof(int)); // Allocate space for 10 integers
	
free
	Prototype:	void free(void *ptr);
	Purpose:	Frees dynamically allocated memory (from malloc or similar functions).
	Parameters:
		ptr: Pointer to the memory block to free.
	Usage:		Prevents memory leaks by deallocating unused memory.
	Example:	free(array); // Deallocate memory allocated with malloc
	
write
	Prototype:	ssize_t write(int fd, const void *buf, size_t count);
	Purpose:	Writes data to a file descriptor (e.g., terminal, file).
	Parameters:
...
usleep
	...
	Purpose:	Suspends program execution for a specified time in microseconds.
	Parameters:
		usec: Number of microseconds to sleep (1 second = 1,000,000 microseconds).
	Usage:		Introduces short delays.
	Example:	usleep(500000); // Sleep for 500 milliseconds (0.5 seconds)
	
gettimeofday
	Prototype:	int gettimeofday(struct timeval *tv, struct timezone *tz);
	Purpose:	Gets the current time in seconds and microseconds since the Unix epoch.
	Parameters:
		tv: Pointer to a struct timeval to store the time.
		tz: Pointer to a struct timezone (usually NULL and unused).
	Usage:		Measure elapsed time or get the current time.
	Example:	struct timeval start;
				gettimeofday(&start, NULL);
	
pthread_create
	Prototype:	int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine)(void *), void *arg);
	Purpose:	Creates a new thread that runs a specific function.
	Parameters:
...
pthread_detach
	...
	Purpose:	Marks a thread for automatic resource cleanup when it terminates.
	Parameters:
		thread: Thread ID.
	Usage:		Used for "fire-and-forget" threads you don't need to join.
	Example:	pthread_detach(thread);
	
pthread_join
	Prototype:	int pthread_join(pthread_t thread, void **retval);
	Purpose:	Waits for a thread to finish execution and optionally retrieves its return value.
	Parameters:
		thread:	Thread ID.
...
pthread_mutex_init
	...
	Purpose:	Initializes a mutex for thread synchronization.
	Parameters:
		mutex:	Pointer to the mutex.
		attr:	Mutex attributes (NULL for default).
	Usage:		Protect shared resources in multi-threaded programs.
	Example:	pthread_mutex_t lock;
				pthread_mutex_init(&lock, NULL);
	
pthread_mutex_destroy
	Prototype:	int pthread_mutex_destroy(pthread_mutex_t *mutex);
	Purpose:	Destroys a mutex and releases its resources.
	Parameters:
		mutex:	Pointer to the mutex.
	Usage:		Called after you no longer need a mutex.
	Example:	pthread_mutex_destroy(&lock);
	
pthread_mutex_lock
	Prototype:	int pthread_mutex_lock(pthread_mutex_t *mutex);
	Purpose:	Locks a mutex, blocking if it’s already locked.
	Parameters:
		mutex:	Pointer to the mutex.
	Usage:		Ensures only one thread can access a critical section at a time.
	Example:	pthread_mutex_lock(&lock);
				// Critical section
				pthread_mutex_unlock(&lock);
	
pthread_mutex_unlock
	Prototype:	int pthread_mutex_unlock(pthread_mutex_t *mutex);
	Purpose:	Unlocks a previously locked mutex.
	Parameters:
		mutex:	Pointer to the mutex.
	Usage:		Allows other threads to acquire the mutex.
	Example:	pthread_mutex_unlock(&lock);




https://github.com/pulgamecanica/42Course/tree/main/42Core/philosophers